...
A drawable's widget tree is compiled to Clay declarations from the root down. Every widget the compile step can express becomes one Clay element, and every subtree it cannot keeps drawing itself with cairo, into a surface of its own that the renderer shows at the box Clay solves for it. `wibox.container.margin`, `wibox.container.background`, `wibox.container.place`, `wibox.layout.fixed`, `wibox.layout.flex`, `wibox.layout.align` and `wibox.layout.stack` convert, and so do `awful.widget.taglist` and `awful.widget.tasklist`, which pass through to their base layout and the items built from their `widget_template`, `wibox.widget.textbox`, which becomes a Clay text element, `wibox.widget.imagebox`, which becomes a Clay image element, `wibox.widget.systray` with its `systray_icon` items, each icon an image element, `awful.widget.clienticon`, an image element of the client's icon, `wibox.container.constraint`, a Clay size limit, and naughty's default box (`naughty.container.background`, `naughty.widget.icon`, `naughty.widget.title`, `naughty.widget.message` and `naughty.list.actions`) under `awful.popup`, whose tree sizes the drawin; everything else, for now, does not. Clay is the only solver of a converted tree: the `:fit` and `:layout` protocol runs only inside the subtrees that draw themselves, and a converted widget is never asked. Widget code and signals are unchanged either way, and so is what a wibar looks like, with the exceptions below.
...
- `visible = false` or an `opacity` other than 1
- `background`: a gradient, a surface pattern or a `bgimage` (which is what a theme's `taglist_squares_sel` and `taglist_squares_unsel` are, so each tag item draws itself when they are set); a shape that is not a rectangle or a rounded rectangle, where a rounded rectangle is `gears.shape.rounded_rect` itself or a function that draws its path with one radius, which is what a theme's `function(cr, w, h) gears.shape.rounded_rect(cr, w, h, r) end` is; a rounded shape together with a border width
...
- `imagebox`: an SVG (rendered at the screen's dpi), a `clip_shape`, a `horizontal_fit_policy` or `vertical_fit_policy` other than `"auto"`, `upscale` or `downscale` set to false, a `max_scaling_factor`, or a `:draw` override
- `systray`: `beautiful.systray_max_rows` above 1, a fractional `systray_paddings` or spacing, or a `bg_systray` that is not a solid color
...
**A rounded background cuts its children to the arc.** A `background` whose shape is a rounded rectangle converts with whatever it holds, filled or not, and the renderer clips every node under it to the shape's box and arc, as the container's own cairo clip did. Clipping is the renderer's rather than Clay's: a Clay clip element is a scroll container, and a context holds ten of them, so no converted drawin or container is one, and the count of converted wiboxes on an output is bounded by the element budget alone.

**A rounded wibox is a rounded root element.** A `shape` that draws a rounded rectangle (`beautiful.wibar_shape` in the bundled themes) is read off its masks and becomes the corner radius of the drawable's own background, and the renderer cuts anything drawn into a corner to the arc, as the mask cut it. A `background` border's straight edges are the one thing not cut: a bordered widget touching a rounded corner shows its edge past the arc. The corners take no input, as before: a pointer there falls through to what is behind the wibox.

**A popup's tree sizes its drawin.** An `awful.popup` sizes itself to its widget: under Clay the drawin's root element wraps the tree (`CLAY_SIZING_FIT`) within the popup's `minimum_width`, `maximum_width`, `minimum_height` and `maximum_height`, and the popup takes the box Clay solves, as it took the engine's fit, one frame after the tree changes.


**A widget that draws itself is sized by its `:fit`, within the bound Clay's solve leaves it.** The tree is compiled with every such widget asked at the drawable's size, solved, and each widget asked again within the box the nearest layout with a direction got, minus the padding between and what that layout's other children took; when an answer changed, the tree solves once more. On an axis where the widget takes one more pixel when offered one more, it grows into what the layout leaves instead. A widget whose size settles only after more than one such round is placed at the second answer.

**A forced size is Clay's, where a parent asks for it.** `forced_width` and `forced_height` on a converted widget convert with it: a `fixed` or `align` slot places the widget at that size, and a container that hands its child the whole box (`margin`, `background`) still gives it the whole box, counting the forced size as the least the box can be.
...
**A slot that grows keeps content wider than its share.** The engine gives every `flex` child the same share whatever it holds, and gives `align`'s expanded slots what the fixed ones leave. Clay grows an element up from the size of its own content and never compresses it below that, so a slot whose content is already wider than its share keeps that width and the slots beside it get what is left. An `align` whose middle widget wants the whole length no longer drops the outer two; it places all three and the middle takes what they leave.
