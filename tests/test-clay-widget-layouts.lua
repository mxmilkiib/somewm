...
local steps = {
    ...
    function()
        ...
        -- under it, and the topmost is the clock. Warped twice, since the
        -- hit test runs against the position before the move.
        mouse.coords({ x = BX + 200, y = BY + 12 })
        mouse.coords({ x = BX + 200, y = BY + 12 })
        assert(mouse.current_widget == clock, "the clock is not the current widget")
        assert(#mouse.current_widgets == 5, #mouse.current_widgets .. " current widgets, want 5")
        mouse.coords({ x = 100, y = 100 })
