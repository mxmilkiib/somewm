...
client_wipe(client_t *c)
{
    ...
    /* TODO: When we fully implement key bindings, add proper destructor for each key */
    if (c->keys.tab) {
        /* For now, just free the array - proper implementation needs key destructor */
        free(c->keys.tab);
        c->keys.tab = NULL;
        c->keys.len = c->keys.size = 0;
    }

    /* X11 protocols cleanup - Wayland doesn't use this, just free if allocated */
    if (c->protocols.atoms) {
        free(c->protocols.atoms);
        c->protocols.atoms = NULL;
        c->protocols.atoms_len = 0;
    }

    /* TODO: When we fully implement icons, add proper cairo_surface cleanup for each icon */
    if (c->icons.tab) {
        /* For now, just free the array - proper implementation needs cairo_surface_destroy for each */
        free(c->icons.tab);
        c->icons.tab = NULL;
 ...
 */
/* ========================================================================
 * Array-Based Visibility (AwesomeWM Pure Implementation)
 * ======================================================================== */

/** Check if client is visible on selected tags
 * Pure AwesomeWM implementation using tag arrays without bitmasks.
 * Iterates through all tags and checks if:
 * 1. Tag is selected
 * 2. Tag belongs to client's screen
 * 3. Client is in tag's clients array
 *
 * \param c The client to check
...
client_on_selected_tags(client_t *c)
{
    ...
    /* Iterate all tags, check if selected and contains this client */
    for(int i = 0; i < globalconf.tags.len; i++) {
        tag_t *tag = globalconf.tags.tab[i];

        /* Skip tags not on this client's screen */
        if(tag->screen != client_screen)
            continue;

        /* Skip unselected tags */
        if(!tag->selected)
            continue;

        /* Check if client is in this tag's client array */
...
/** Check if two clients share any tags (array-based)
 * Replaces bitmask check: (c1->tags & c2->tags)
 *
 * \param c1 First client
 * \param c2 Second client
 * \return true if clients share at least one tag, false otherwise
 */
bool
clients_share_tags(client_t *c1, client_t *c2)
{
    int i, j;

    /* Sticky clients are on all tags */
    if(c1->sticky || c2->sticky)
        return true;

    /* Iterate through all tags */
    for(i = 0; i < globalconf.tags.len; i++) {
        tag_t *tag = globalconf.tags.tab[i];
        bool c1_has_tag = false;
        bool c2_has_tag = false;

        /* Check if c1 is in this tag */
        for(j = 0; j < tag->clients.len; j++) {
            if(tag->clients.tab[j] == c1) {
                c1_has_tag = true;
                break;
            }
        }

        /* Check if c2 is in this tag */
        for(j = 0; j < tag->clients.len; j++) {
            if(tag->clients.tab[j] == c2) {
                c2_has_tag = true;
                break;
            }
        }

        /* If both clients are in this tag, they share it */
        if(c1_has_tag && c2_has_tag)
            return true;
    }
...
client_resize_do(client_t *c, area_t geometry, bool silent)
{
    ...
    for (client_titlebar_t bar = CLIENT_TITLEBAR_TOP; bar < CLIENT_TITLEBAR_COUNT; bar++) {
        ...
        luaA_object_push(L, c);
        drawable = titlebar_get_drawable(L, c, -1, bar);
...
client_unmanage(client_t *c, client_unmanage_t reason)
{
    ...
    /* Only perform XCB operations on X11/XWayland clients */
    if(reason != CLIENT_UNMANAGE_DESTROYED && c->client_type == X11)
    {
    ...
    if(reason != CLIENT_UNMANAGE_DESTROYED && c->client_type == X11)
    {
        ...
        geometry = client_get_undecorated_geometry(c);
    ...
    /* Invalidate client (matches AwesomeWM timing: AFTER signals).
     * For XDGShell clients, clear surface pointer. For X11 clients, clear window. */
    if (c->client_type == XDGShell) {
        c->surface.xdg = NULL;
    }
#ifdef XWAYLAND
    else if (c->client_type == X11) {
        /* X11 window invalidation handled below */
    }
#endif
...
titlebar_resize(lua_State *L, int cidx, client_t *c, client_titlebar_t bar, int size)
{
    ...
    geometry = c->geometry;
    change = size - c->titlebar[bar].size;
    switch (bar) {
    case CLIENT_TITLEBAR_TOP:
...
{                                                                 \
    ...
    {                                                             \
        if (lua_isnil(L, 2))                                      \
            ...
            titlebar_resize(L, 1, c, index, (int)ceil(luaA_checknumber_range(L, 2, 0, MAX_X11_SIZE))); \
    }                                                             \
                                                                  \
...
luaA_client_apply_size_hints(lua_State *L)
{
    ...
    if(!client_isfixed(c))
    {
        geometry.width = (int)ceil(luaA_checknumber_range(L, 2, MIN_X11_SIZE, MAX_X11_SIZE));
        geometry.height = (int)ceil(luaA_checknumber_range(L, 3, MIN_X11_SIZE, MAX_X11_SIZE));
    }
 ...
 * This includes the XDG surface content.
...
luaA_client_get_icon(lua_State *L, client_t *c)
{
    ...
    found = NULL;
    found_size = 0;
    preferred_size = globalconf.preferred_icon_size;
...
{
    /* For native Wayland clients (XDGShell), check if surface exists */
    if (c->client_type == XDGShell)
        return c->surface.xdg != NULL;

#ifdef XWAYLAND
    /* For X11 clients, check if window ID exists */
    if (c->client_type == X11)
        return c->window != XCB_NONE;
#endif

    /* For other types (LayerShell, etc.), assume valid if we have the struct */
    return true;
}
